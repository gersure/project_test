...
#define _make_histogram_family(val) \
    val(STORE_ENGINE_COMPACTION_DURATIONS_VEC,  "engine_compaction_duration_seconds",   "Histogram of compaction duration seconds",     {"db", "cf"},   exponential_buckets(0.005, 2.0, 20))
...
/*
#define _make_counter_familys(val) \
    val(STORE_ENGINE_CACHE_EFFICIENCY_VEC,      "engine_cache_efficiency",      "Efficiency of rocksdb's block cache") 		\
    val(STORE_ENGINE_MEMTABLE_EFFICIENCY_VEC,   "engine_memtable_efficiency",   "Hit and miss of memtable") 				\
    val(STORE_ENGINE_GET_SERVED_VEC,            "engine_get_served",            "Get queries served by engine") 			\
    val(STORE_ENGINE_WRITE_SERVED_VEC,          "engine_write_served",          "Write queries served by engine") 			\
    val(STORE_ENGINE_BLOOM_EFFICIENCY_VEC,      "engine_bloom_efficiency",      "Efficiency of rocksdb's bloom filter") 	\
    val(STORE_ENGINE_FLOW_VEC,                  "engine_flow_bytes",            "Bytes and keys of read/written") 			\
    val(STORE_ENGINE_STALL_MICROS,              "engine_stall_micro_seconds",   "Stall micros")         					\
    val(STORE_ENGINE_COMPACTION_FLOW_VEC,       "engine_compaction_flow_bytes", "Bytes of read/written during compaction") 	\
    val(STORE_ENGINE_COMPACTION_DROP_VEC,       "engine_compaction_key_drop",   "Count the reasons for key drop during compaction") 	\
    val(STORE_ENGINE_COMPACTION_NUM_CORRUPT_KEYS_VEC, "engine_compaction_num_corrupt_keys", "Number of corrupt keys during compaction")	\
    val(STORE_ENGINE_COMPACTION_REASON_VEC,     "engine_compaction_reason",     "Number of compaction reason") 				\
    val(STORE_ENGINE_LOCATE_VEC,                "engine_locate",                "Number of calls to seek/next/prev") 		\
    val(STORE_ENGINE_FILE_STATUS_VEC,           "engine_file_status",           "Number of different status of files") 		\
    val(STORE_ENGINE_READ_AMP_FLOW_VEC,         "engine_read_amp_flow_bytes",   "Bytes of read amplification") 				\
    val(STORE_ENGINE_NO_ITERATORS,              "engine_no_iterator",           "Number of iterators currently open")		\
    val(STORE_ENGINE_WAL_FILE_SYNCED,           "engine_wal_file_synced",       "Number of times WAL sync is done")         \
    val(STORE_ENGINE_EVENT_COUNTER_VEC,         "engine_event_total",           "Number of engine events")


#define _make_gauge_familys(val) \
    val(STORE_ENGINE_SIZE_GAUGE_VEC,                "engine_size_bytes",                "Sizes of each column families") 				\
    val(STORE_ENGINE_BLOCK_CACHE_USAGE_GAUGE_VEC,   "engine_block_cache_size_bytes",    "Usage of each column families' block cache")   \
    val(STORE_ENGINE_MEMORY_GAUGE_VEC,              "engine_memory_bytes",              "Sizes of each column families")   				\
    val(STORE_ENGINE_ESTIMATE_NUM_KEYS_VEC,         "engine_estimate_num_keys",         "Estimate num keys of each column families")	\
    val(STORE_ENGINE_GET_MICROS_VEC,                "engine_get_micro_seconds",         "Histogram of get micros")         				\
    val(STORE_ENGINE_WRITE_MICROS_VEC,              "engine_write_micro_seconds",       "Histogram of write micros")         			\
    val(STORE_ENGINE_COMPACTION_TIME_VEC,           "engine_compaction_time",           "Histogram of compaction time")         		\
    val(STORE_ENGINE_TABLE_SYNC_MICROS_VEC,         "engine_table_sync_micro_seconds",  "Histogram of table sync micros")         		\
    val(STORE_ENGINE_COMPACTION_OUTFILE_SYNC_MICROS_VEC,    "engine_compaction_outfile_sync_micro_seconds",     "Histogram of compaction outfile sync micros") 	\
    val(STORE_ENGINE_MANIFEST_FILE_SYNC_MICROS_VEC, "engine_manifest_file_sync_micro_seconds",                  "Histogram of manifest file sync micros") 		\
    val(STORE_ENGINE_WAL_FILE_SYNC_MICROS_VEC,      "engine_wal_file_sync_micro_seconds",                       "Histogram of WAL file sync micros") 			\
    val(STORE_ENGINE_STALL_L0_SLOWDOWN_COUNT_VEC,   "engine_stall_l0_slowdown_count",                           "Histogram of stall l0 slowdown count") 		\
    val(STORE_ENGINE_STALL_MEMTABLE_COMPACTION_COUNT_VEC,   "engine_stall_memtable_compaction_count",           "Histogram of stall memtable compaction count") \
    val(STORE_ENGINE_STALL_LO_NUM_FILES_COUNT_VEC,          "engine_stall_l0_num_files_count",                  "Histogram of stall l0 num files count") 		\
    val(STORE_ENGINE_HARD_RATE_LIMIT_DELAY_COUNT_VEC,       "engine_hard_rate_limit_delay_count",               "Histogram of hard rate limit delay count") 	\
    val(STORE_ENGINE_SOFT_RATE_LIMIT_DELAY_COUNT_VEC,       "engine_soft_rate_limit_delay_count",               "Histogram of soft rate limit delay count") 	\
    val(STORE_ENGINE_NUM_FILES_IN_SINGLE_COMPACTION_VEC,    "engine_num_files_in_single_compaction",            "Histogram of number of files in single compaction") \
    val(STORE_ENGINE_SEEK_MICROS_VEC,               "engine_seek_micro_seconds",        "Histogram of seek micros")					\
    val(STORE_ENGINE_WRITE_STALL_VEC,               "engine_write_stall",               "Histogram of write stall") 				\
    val(STORE_ENGINE_SST_READ_MICROS_VEC,           "engine_sst_read_micros",           "Histogram of SST read micros") 			\
    val(STORE_ENGINE_NUM_SUBCOMPACTION_SCHEDULED_VEC,       "engine_num_subcompaction_scheduled",               "Histogram of number of subcompaction scheduled") 	\
    val(STORE_ENGINE_BYTES_PER_READ_VEC,            "engine_bytes_per_read",            "Histogram of bytes per read") 				\
    val(STORE_ENGINE_BYTES_PER_WRITE_VEC,           "engine_bytes_per_write",           "Histogram of bytes per write") 			\
    val(STORE_ENGINE_BYTES_COMPRESSED_VEC,          "engine_bytes_compressed",          "Histogram of bytes compressed") 			\
    val(STORE_ENGINE_BYTES_DECOMPRESSED_VEC,        "engine_bytes_decompressed",        "Histogram of bytes decompressed") 			\
    val(STORE_ENGINE_COMPRESSION_TIMES_NANOS_VEC,   "engine_compression_time_nanos",    "Histogram of compression time nanos") 		\
    val(STORE_ENGINE_DECOMPRESSION_TIMES_NANOS_VEC, "engine_decompression_time_nanos",  "Histogram of decompression time nanos") 	\
    val(STORE_ENGINE_PENDING_COMACTION_BYTES_VEC,   "engine_pending_compaction_bytes",  "Pending compaction bytes")   				\
    val(STORE_ENGINE_COMPRESSION_RATIO_VEC,         "engine_compression_ratio",         "Compression ratio at different levels") 	\
    val(STORE_ENGINE_NUM_SNAPSHOTS_GAUGE_VEC,       "engine_num_snapshots",             "Number of unreleased snapshots") 			\
    val(STORE_ENGINE_OLDEST_SNAPSHOT_DURATION_GAUGE_VEC,    "engine_oldest_snapshot_duration",                  "Oldest unreleased snapshot duration in seconds") \
    val(STORE_ENGINE_NUM_FILES_AT_LEVEL_VEC,        "engine_num_files_at_level",        "Number of files at each level")      				\
    val(STORE_ENGINE_NUM_IMMUTABLE_MEM_TABLE_VEC,   "engine_num_immutable_mem_table",   "Number of immutable mem-table")               		\
    val(STORE_ENGINE_STALL_CONDITIONS_CHANGED_VEC,  "engine_stall_conditions_changed",  "Stall conditions changed of each column family")

#define _make_histogram_familys(val) \
    val(STORE_ENGINE_COMPACTION_DURATIONS_VEC,  "engine_compaction_duration_seconds",   "Histogram of compaction duration seconds",   exponential_buckets(0.005, 2.0, 20))
*/



/*
#define _make_counter_familys_param(param, name, help, ...)   \
    prometheus::Family<prometheus::Counter>& param;
    _make_counter_familys(_make_counter_familys_param)

#define _make_gauge_familys_param(param, name, help, ...)   \
    prometheus::Family<prometheus::Gauge>& param;
    _make_gauge_familys(_make_gauge_familys_param)


#define _make_histogram_familys_param(param, name, help, ...)   \
    prometheus::Family<prometheus::Histogram>& param;
    _make_histogram_familys(_make_histogram_familys_param)
*/
